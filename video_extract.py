...
for batch_i, (video_id, img_paths, batch_of_input_imgs) in enumerate(dataloader):
    ...
    with torch.no_grad():
        ...
        detections_point5 = non_max_suppression(detections_raw, 80, 0.5, opt.nms_thres)
        detections_point8 = non_max_suppression(detections_raw, 80, 0.8, opt.nms_thres)
        detections = detections_point8

        detections_point8 = detections_point8.permute(0, 2, 1)

        print (torch.stack(detections_point8).shape)
        print (f"Saving the video id {video_id}")
        # torch.save(torch.stack(detections_raw), os.path.join(f"{opt.output_folder}/raw", f'{video_id}.pt'))
